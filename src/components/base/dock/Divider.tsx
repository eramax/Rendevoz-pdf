...
class BoxDataCache implements DividerData {
  ...
  constructor(data: DividerData) {
    ...
    for (const child of this.beforeDivider) {
      this.beforeSize += child.size
      if (child.minSize > 0) {
        this.beforeMinSize += child.minSize
      }
    }
    for (const child of this.afterDivider) {
...
function spiltSize(newSize: number, oldSize: number, children: DividerChild[]): number[] {
  ...
  while (requiredMinSize !== reservedSize) {
    ...
    requiredMinSize = 0
    const ratio = (newSize - reservedSize) / (oldSize - reservedSize)
    if (!(ratio >= 0)) {
      // invalid input
      break
    }
...
export class Divider extends React.PureComponent<DividerProps, any> {
  ...
  dragMove2(e: DragState, dx: number, dy: number) {
    const { isVertical, changeSizes } = this.props
    const { beforeDivider, afterDivider } = this.boxData
    ...
    const rightCild = afterDivider[0]
    ...
    const sizes = beforeDivider.concat(afterDivider).map(child => child.size)
  ...
  dragMoveAll(e: DragState, dx: number, dy: number) {
    const { isVertical, changeSizes } = this.props
    const { beforeSize, beforeMinSize, afterSize, afterMinSize, beforeDivider, afterDivider } = this.boxData
    const d = isVertical ? dy : dx
  ...
  render(): React.ReactNode {
    ...
    return <DragDropDiv className={className} onDragStartT={this.startDrag} onDragMoveT={this.dragMove} onDragEndT={this.dragEnd} />
  }
}
