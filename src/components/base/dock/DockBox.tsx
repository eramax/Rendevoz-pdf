...
export class DockBox extends React.PureComponent<Props, any> {
  ...
  getDividerData = (idx: number) => {
    if (this._ref) {
      const { children, mode } = this.props.boxData
      const nodes = this._ref.childNodes
      if (nodes.length === children.length * 2 - 1) {
        const dividerChildren: DividerChild[] = []
  ...
  render(): React.ReactNode {
    ...
    const { minWidth, minHeight, size, children, mode, id, widthFlex, heightFlex } = boxData
    ...
    return (
      <div ref={this.getRef} className={cls} data-dockid={id} style={{ minWidth, minHeight, flex: `${flexGrow} ${flexShrink} ${size}px` }}>
        {childrenRender}
      </div>
    )
  }
