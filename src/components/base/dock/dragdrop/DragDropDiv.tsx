...
export class DragDropDiv extends React.PureComponent<DragDropDivProps, any> {
  ...
  _getRef = (r: HTMLDivElement) => {
    ...
    const { getRef, onDragOverT } = this.props
  ...
  onPointerDown = (e: React.MouseEvent | React.TouchEvent) => {
    ...
    const { onDragStartT, onGestureStartT, onGestureMoveT, useRightButtonDragT } = this.props
  ...
  onDragStart(event: MouseEvent | TouchEvent) {
    ...
    const state = new DragManager.DragState(event, this, true)
    this.baseX = state.pageX
    this.baseY = state.pageY

    const baseElement = this.element.parentElement
  ...
  checkFirstMove(e: AbstractPointerEvent) {
    const state = new DragManager.DragState(e, this, true)
    if (!state.moved()) {
      // not a move
      return false
    }
  ...
  onMouseMove = (e: MouseEvent) => {
    ...
    if (this.waitingMove) {
      ...
      const state = new DragManager.DragState(e, this)
      state._onMove()
      if (onDragMoveT) {
        onDragMoveT(state)
      }
  ...
  onGestureStart(event: TouchEvent) {
    ...
    const { onGestureStartT } = this.props
    ...
    const state = new GestureState(event, this, true)
  ...
  onGestureMove = (e: TouchEvent) => {
    ...
    const state = new GestureState(e, this)
    if (this.waitingMove) {
  ...
  onGestureEnd = (e?: TouchEvent) => {
    const { onGestureEndT } = this.props
    const state = new DragManager.DragState(e, this)
  ...
  render(): React.ReactNode {
    ...
    return (
      <div ref={this._getRef} className={className} {...others} onMouseDown={onMouseDown} onTouchStart={onTouchDown}>
        {children}
      </div>
    )
  }
  ...
  componentDidUpdate(prevProps: DragDropDivProps) {
    const { onDragOverT, onDragEndT, onDragLeaveT } = this.props
    if (
      this.element &&
      (prevProps.onDragOverT !== onDragOverT || prevProps.onDragLeaveT !== onDragLeaveT || prevProps.onDragEndT !== onDragEndT)
    ) {
      if (onDragOverT) {
